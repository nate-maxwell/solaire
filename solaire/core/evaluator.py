"""
Environment handling for the sys path and code execution.

This is necessary for running project code and managing runtime namespaces
of imported modules from user-generated code.
"""


import sys
from pathlib import Path
from typing import Any
from typing import Optional


_PROJECT_PATH: Optional[Path] = None
_USER_NAMESPACE: dict[str, Any] = {}
_INITIAL_MODULES: set[str] = set(sys.modules.keys())


def update_project_path(path: Path) -> None:
    """Update the project path and add it to sys.path."""
    global _PROJECT_PATH

    # Remove old project path if it exists
    if _PROJECT_PATH and _PROJECT_PATH.as_posix() in sys.path:
        sys.path.remove(_PROJECT_PATH.as_posix())

    if path.exists():
        _PROJECT_PATH = path
    else:
        raise ValueError(f'New project path does not exist {path}!')

    sys.path.insert(0, path.as_posix())

    clear_user_modules()


def clear_user_modules() -> None:
    """
    Remove all user-imported modules from sys.modules.

    This keeps only the modules that were loaded when the IDE started,
    allowing clean imports when switching projects.
    """
    # Get modules that were added after IDE startup
    current_modules = set(sys.modules.keys())
    user_modules = current_modules - _INITIAL_MODULES

    # Remove user-imported modules
    for module_name in user_modules:
        # Skip built-in modules and IDE-critical modules
        if not module_name.startswith('_') and module_name in sys.modules:
            del sys.modules[module_name]


def clear_namespace() -> None:
    """Clear the user execution namespace."""
    global _USER_NAMESPACE
    _USER_NAMESPACE.clear()


def execute_user_code(code: str) -> Optional[str]:
    """Execute user code in a persistent namespace that maintains state
    between executions, similar to a Python REPL or Jupyter notebook.

    Args:
        code (str): The Python code to execute
    Returns:
        Optional[str]: None if successful, error message string if execution
            failed.
    """
    global _USER_NAMESPACE

    try:
        # Execute code in the persistent user namespace
        exec(code, _USER_NAMESPACE)
        return None
    except Exception as e:
        # Return the error message for display in the IDE
        error_type = type(e).__name__
        error_message = str(e)
        return f'{error_type}: {error_message}'


def get_namespace() -> dict[str, Any]:
    """Get the current user namespace.

    Useful for inspecting variables, functions, and classes
    defined by user code.
    """
    return _USER_NAMESPACE.copy()


def reset_environment() -> None:
    """Completely reset the execution environment.

    Clears both the namespace and user-imported modules.
    Use when switching projects or when a clean slate is needed.
    """
    clear_namespace()
    clear_user_modules()
