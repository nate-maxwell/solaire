...
class YamlHighlighter(QtGui.QSyntaxHighlighter):
    ...
    def __init__(self, parent: QtGui.QTextDocument | None = None) -> None:
        ...
        rules.append(
            QtWrappers.HighlightRule(block_scalar_pattern, _color_scheme.indicator, group=1)
        )
        ...
        tag_pattern = r'(![!\w][\w./-]*)'
        rules.append(
            QtWrappers.HighlightRule(tag_pattern, _color_scheme.tag, group=1)
        )
        ...
        key_pattern = r'^(?:\s*-\s+)?(("|\')?[^"\'\s\-{}\[\],&*!#][^:\n]*?\2)\s*:(?=\s|$)'
        rules.append(
            QtWrappers.HighlightRule(key_pattern, _color_scheme.key, group=1)
        )
        ...
        rules.append(
            QtWrappers.HighlightRule(r':\s*("[^"\\]*(?:\\.[^"\\]*)*")', _color_scheme.value, group=1)
        )
        rules.append(
            QtWrappers.HighlightRule(r":\s*('[^'\\]*(?:\\.[^'\\]*)*')", _color_scheme.value, group=1)
        )
    ...
    def highlightBlock(self, text: str) -> None:
        ...
        for rule in self.rules:
            ...
            while it.hasNext():
                ...
                if comment_start >= 0 and start >= comment_start and rule.format is not _color_scheme.comment:
                    continue
