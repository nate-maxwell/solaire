...
class CodeStructureWidget(QtWidgets.QTreeWidget):
    ...
    def _create_subscriptions(self) -> None:
        ...
        broker.register_subscriber(
            "tab_manager", "active_changed", self._tab_changed_subscription
        )
        broker.register_subscriber(
            "tab_manager", "all_tabs_closed", self._all_tabs_closed_subscription
        )
    ...
    def update_structure(self, code: str) -> None:
        ...
        try:
            ...
            for name in names:
                if name.type == "class":
                    classes[name.name] = {"item": name, "methods": []}
            ...
            sorted_classes = sorted(
                classes.items(), key=lambda kv: (kv[1]["item"].line, kv[0])
            )
            ...
            for class_name, class_data in sorted_classes:
                ...
                class_item.setData(
                    0, QtCore.Qt.ItemDataRole.UserRole, class_data["item"].line
                )
                ...
                for method in sorted(class_data["methods"], key=lambda x: x.line):
                    method_item = QtWidgets.QTreeWidgetItem(class_item)
                    method_item.setText(0, f"  🔧 {method.name}()")
                    method_item.setData(0, QtCore.Qt.ItemDataRole.UserRole, method.line)
            ...
            for func in sorted(functions, key=lambda x: x.line):
                ...
                func_item.setData(0, QtCore.Qt.ItemDataRole.UserRole, func.line)
